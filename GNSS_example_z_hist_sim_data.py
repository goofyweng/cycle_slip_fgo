"""
This script is intented to verify the observation model using simulated
observations by checking the histogram of weighted residual norm, 'z'.
The histogram is overlaped with the theoretical chi-squared PDF of z, 
and the calculated threshold for fault detection, 'T'.
Modify 'num_files' variable to choose the number of files to be processed.
"""
...
from GNSS_code_TDCP_model import build_h_A, build_h_A_num_sats_sim_obs
...
if __name__ == "__main__":
    ...
    for k in k_vec:
        # do the calculation for iter_num times
        for epoch_pair in chosen_epochs_filtered:
            ...
            (
                ...
                h_A,  # (A @ h) function
                sat_ele_rad,
            ) = build_h_A_num_sats_sim_obs(data_gps_c1c, epoch_pair, k)
            ...
            sigma_code = 3
            sigma_phase = 0.05
            code_noise = np.random.normal(0, sigma_code, y[: 2 * k].shape)
            phase_noise = np.random.normal(0, sigma_phase, y[2 * k :].shape)
            # add observation noise to code measurement
            y[: 2 * k] = y[: 2 * k] + code_noise
            y[2 * k :] = y[2 * k :] + phase_noise
    ...
    np.savez("result_sim_data.npz", k_vec=k_vec, z_vec=z_vec, T_vec=T_vec, dof=dof)
    ...
    for j in range(k_vec.shape[0]):
        ...
        plot_non_central_chi2(ax[row, col], dof[j], 0, xlim=60)
        ...
        ax[row, col].hist(
            ...
            alpha=0.5,
            edgecolor="black",
            label=f"$\\text{{z}}$, # of counts={np.sum(~np.isnan(z_vec[j,:]))}",
        )
        ax[row, col].legend()
